...
const DEFAULT_ITEMS: UserReadme[] = Array.from({ length: 15 }, (_, i) => {
  const ids = [1015, 1025, 1039, 1043, 1044, 1050, 1062, 1069, 1074, 1080, 1084, 106, 110, 133, 164];
  return {
    avatar_url: `https://picsum.photos/id/${ids[i % ids.length]}/600/400`,
    username: `Tile ${i + 1}`,
    url: ""
  };
});
...
const DriftWall = ({
  ...
  const columnItems = useMemo<UserReadme[][]>(() => {
    const cols: UserReadme[][] = Array.from({ length: columns }, () => []);
  ...
  const renderTile = (item: UserReadme, id: string, colIndex: number) => {
    const inner = (
    ...
    if (item.url) {
      return (
        <a key={id} href={item.url} target="_blank" rel="noreferrer noopener" {...commonProps}>
          {inner}
        </a>
      );
    }
    return (
      <div key={id} tabIndex={0} role="button" aria-label={item.username ?? 'tile'} {...commonProps}>
        {inner}
      </div>
    );
  };
